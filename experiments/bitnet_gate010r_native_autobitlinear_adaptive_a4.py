...
from transformers.integrations.bitnet import AutoBitLinear, ActQuant
...
print("GATE010R_START",json.dumps({
 ...
 "adaptive_codebook_entries_per_AutoBitLinear":CODEBOOK,"scale_index_bits":SCALE_BITS,
 "selection_mse_channel_sample":MSE_CH,"bootstrap_resamples":BOOT,
 "repair":"target offline AutoBitLinear forward activation site instead of BitLinear.activation_quant",
...
if any(bool(m.online_quant) for _,m in bitmods):
    raise RuntimeError("Expected offline AutoBitLinear modules, found online_quant=True")
...
orig_forwards=[m.forward for _,m in bitmods]
...
for i,(_,m) in enumerate(bitmods):
    def make_collect(ii):
        def forward(self,input):
            ...
            target_banks[ii].append(oracle_target_clip(input))
            qinput=ActQuant.apply(input)
            output=F.linear(qinput,self.weight,self.bias)
            return output*self.weight_scale
        return forward
    m.forward=types.MethodType(make_collect(i),m)
...
try:
    ...
    for (_,m),orig in zip(bitmods,orig_forwards):
        m.forward=orig
...
def adaptive_a4_dequant(x,cb_cpu):
    dtype=x.dtype
    xf=x.float()
    xs=sample_channels(xf)
...
def make_mode_forward(mode,cb_cpu=None):
    def forward(self,input):
        if self.rms_norm is not None:
            input=self.rms_norm(input)
        if mode=="dynamic_a4":
            qinput=dynamic_a4_dequant(input)
        elif mode=="adaptive_a4":
            qinput=adaptive_a4_dequant(input,cb_cpu)
        else:
...
def patch_mode(mode):
    ...
    if mode=="dynamic_a4":
        for _,m in bitmods:
            m.forward=types.MethodType(make_mode_forward("dynamic_a4"),m)
        return
    if mode=="adaptive_a4":
        for (_,m),cb in zip(bitmods,codebooks):
            m.forward=types.MethodType(make_mode_forward("adaptive_a4",cb),m)
        return
    raise ValueError(mode)
...
status="GATE010R_NATIVE_AUTOBITLINEAR_ADAPTIVE_A4_PASS" if robust else "GATE010R_NATIVE_AUTOBITLINEAR_ADAPTIVE_A4_FAIL_STOP_ADAPTIVE_METHOD_LANE"
final={
 "experiment":"BITNET_GATE010R_NATIVE_AUTOBITLINEAR_ADAPTIVE_A4_TRANSFER",
 "status":status,
 "repair_boundary":"Implementation-only repair of Gate010: target actual offline AutoBitLinear activation site. Scientific protocol and thresholds unchanged.",
 "config":{
   "model":MODEL,"seed":SEED,"seq":SEQ,"calibration_tokens":N_CAL*SEQ,
   "evaluation_sequences_per_corpus":N_EVAL,
   "evaluation_prediction_tokens_per_corpus":N_EVAL*(SEQ-1),
   "autobitlinear_modules":len(bitmods),"codebook_entries":CODEBOOK,
...
with open("gate010r_results.json","w") as f:
