...
def build_complex():
    ...
    outer = sorted({
        v for tet in star_tets for v in tet
        if v != (0, 0, 0)
    })
    ...
    boundary_edges = {
        tuple(sorted(e))
        for q in boundary_tets
        for e in itertools.combinations(q, 2)
    }
...
def initial_length_dict(h, amp=1.0):
    return {
        e: edge_len_rnc(e[0], e[1], h, amp=amp)
        for e in CX["all_edges"]
    }
...
def simplex_dihedral_angles(simplex, lengths):
    ...
    for i, j in itertools.combinations(range(5), 2):
        ...
        theta = math.acos(c)
        hinge = tuple(sorted(
            simplex[k] for k in range(5) if k not in (i, j)
        ))
        out[hinge] = theta
    return out
...
def main():
    ...
    x_flat = np.array(
        [flat_lengths[e] / h_ref for e in CX["internal_edges"]],
        dtype=float,
    )
    f_flat = edge_equations_x(x_flat, h_ref, amp=0.0)
    J_flat = jacobian_central(
        lambda z: edge_equations_x(z, h_ref, amp=0.0),
        x_flat,
        eps=1e-6,
    )
    ...
    results = {
        "description": (
            "Independent clean-room FK48 length-Regge pseudo-constraint "
            "replication with a fresh explicit Weyl-free Ricci seed."
        ),
        "seed": {
            "ricci": RICCI.tolist(),
    ...
    for h in H_VALUES:
        ...
        x0 = np.array(
            [lengths0[e] / h for e in CX["internal_edges"]],
            dtype=float,
        )
        ...
        sol = least_squares(
            lambda z: edge_equations_x(z, h, amp=1.0),
            x0,
            xtol=1e-12,
            ftol=1e-12,
            gtol=1e-12,
        ...
        gnorm = float(np.linalg.norm(gauge_part))
        gauge_norms.append(gnorm)
        ...
        results["refinement"].append({
            ...
            "physical_fraction_of_total": float(
                np.linalg.norm(phys_part) / np.linalg.norm(f)
            ),
    ...
    print("Flat four smallest singular values:",
          results["flat_control"]["four_smallest_singular_values"])
    print()
    for row in results["refinement"]:
        print(
            f"h={row['h']:.3f}  "
            f"||E_g||={row['gauge_projected_residual_norm']:.12e}  "
            f"||E_g||/h^4={row['gauge_residual_over_h4']:.12e}  "
            f"phys/total={row['physical_fraction_of_total']:.3e}"
        )
    print()
    print(
        "Gauge obstruction fit: "
        f"p={exponent:.12f}, R^2={r2:.12f}"
    )
    print("Wrote:", out)
