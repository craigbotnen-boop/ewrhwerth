...
WORK="${1:-/tmp/fdep_kt2c_si_convergence_extension_002}"
...
for ecut in cutoffs:
    write_input(Path('outputs') / f'cutoff_{ecut:03d}Ry_k12.in', f'cutoff_{ecut}_k12', ecut, 12)
for k in kmeshes:
    write_input(Path('outputs') / f'kmesh_{k:02d}_ecut80.in', f'kmesh_{k}_ecut80', 80, k)
PY
...
for input in outputs/cutoff_*Ry_k12.in; do run_case "$input"; done
for input in outputs/kmesh_*_ecut80.in; do run_case "$input"; done
...
cutoff = []
for path in sorted(root.glob('outputs/cutoff_*Ry_k12.out')):
    match = re.search(r'cutoff_(\d+)Ry', path.name)
    row = parse(path)
    row['ecutwfc_Ry'] = int(match.group(1))
    row['kmesh'] = [12, 12, 12]
    cutoff.append(row)
cutoff.sort(key=lambda row: row['ecutwfc_Ry'])

kmesh = []
for path in sorted(root.glob('outputs/kmesh_*_ecut80.out')):
    match = re.search(r'kmesh_(\d+)', path.name)
    row = parse(path)
    row['kmesh'] = [int(match.group(1))] * 3
    row['ecutwfc_Ry'] = 80
    kmesh.append(row)
kmesh.sort(key=lambda row: row['kmesh'][0])
...
result = {
    'document_code': 'FDEP_KT2C_SI_CONVERGENCE_EXTENSION_EXECUTION_002',
    ...
    'prospective_extension': {
        'cutoff_ladder_Ry': [60, 70, 80],
        'cutoff_fixed_kmesh': [12, 12, 12],
        'kmesh_ladder': [[10,10,10],[12,12,12],[14,14,14]],
        'kmesh_fixed_ecutwfc_Ry': 80,
    },
    'thresholds': {
    ...
    'selected_settings_if_pass': {
        'ecutwfc_Ry': 80 if cutoff_pass else None,
        'ecutrho_Ry': 640 if cutoff_pass else None,
        'kmesh': [14,14,14] if kmesh_pass else None,
    },
...
(root / 'reports' / 'convergence_extension_result.json').write_text(json.dumps(result, indent=2) + '\n')
print(json.dumps(result, indent=2))
if result['status'] != 'PASS':
    raise SystemExit(1)
PY

echo "HF_CONVERGENCE_EXTENSION_OVERALL: PASS"
