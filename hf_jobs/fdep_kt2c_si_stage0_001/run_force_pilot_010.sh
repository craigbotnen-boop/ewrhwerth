...
WORK="${1:-/tmp/fdep_kt2c_si_force_pilot_010}"
mkdir -p "$WORK"
cd "$WORK"
...
if len(rows)!=expected or not energy or 'JOB DONE.' not in text:
    raise SystemExit(f'FORCE_PARSE_FAIL {prefix} rows={len(rows)} expected={expected}')
receipt={
    'case':prefix,
    'status':'PASS',
    'atom_count':expected,
...
result={
  'document_code':'FDEP_KT2C_SI_FORCE_PILOT_010',
  'status':'PASS',
