...
all_previews = compact.get("exact_zero_rows", []) + compact.get("top_energy_rows_100", []) + compact.get("near_zero_exploratory_top_100", [])
by_key = {(r["layer"], r["coordinate"]): r for r in all_previews}
candidate_rows = [by_key[k] for k in sorted(candidate_keys) if k in by_key]
if len(candidate_rows) != len(candidate_keys):
    missing = sorted(candidate_keys - set(by_key))
    raise RuntimeError(f"Recovered {len(candidate_rows)} of {len(candidate_keys)} candidate rows; missing={missing}")
